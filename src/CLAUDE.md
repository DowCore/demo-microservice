...
dotnet build Meta.Dow.sln

# Build specific service
dotnet build services/administration/host/Meta.Dow.Administration.HttpApi.Host/Meta.Dow.Administration.HttpApi.Host.csproj
...
cd services/administration/host/Meta.Dow.Administration.HttpApi.Host
dotnet run
...
dotnet test Meta.Dow.sln

# Run tests for specific service
dotnet test services/administration/test/Meta.Dow.Administration.Application.Tests/Meta.Dow.Administration.Application.Tests.csproj
...
cd shared/Meta.Dow.DbMigrator
dotnet run

# Add new migration to a service (example: Administration)
cd services/administration/src/Meta.Dow.Administration.MongoDB
...
When running the application with Aspire (`cd apps/Meta.Dow.AppHost && dotnet run`):

1. Open the Aspire Dashboard (typically http://localhost:15888)
2. Navigate to the "MetaDow-DbMigrator" resource
...
This custom command is configured in `apps/Meta.Dow.AppHost/Program.cs` and provides visual feedback in the dashboard.
...
dotnet format analyzers src/Meta.Dow.sln

# Auto-fix code style issues (IDE* rules)
dotnet format style src/Meta.Dow.sln

# Run all auto-fixes and formatting
dotnet format analyzers src/Meta.Dow.sln && dotnet format style src/Meta.Dow.sln && dotnet csharpier .
...
1. **Administration Service** (`services/administration/`) - Port 7001
   ...
   - Database: `MetaDowAdministrationDb`
...
**AuthServer** (`apps/Meta.Dow.AuthServer/`) - Port 7600
...
- `Meta.Dow.ServiceDefaults`: ASP.NET Core hosting, OpenTelemetry, service discovery, resilience
- `Meta.Dow.Hosting.Shared`: Redis caching, RabbitMQ integration, distributed locking, localization
- `Meta.Dow.Microservice.Shared`: JWT auth config, CORS, Swagger setup for microservices
- `Meta.Dow.Shared`: Centralized constants (`MetaDowNames` for service/database names)
- `Meta.Dow.DbMigrator`: Centralized migration tool for all service databases
...
- Exchange: "Meta.Dow", per-service client names
...
- Services resolve each other by name (e.g., "MetaDowAdministration")
...
{
  "ConnectionStrings": {
    "MetaDowAdministrationDb": "Server=localhost;Port=5432;Database=MetaDowAdministrationDb;...",
    "MetaDowIdentityServiceDb": "Server=localhost;Port=5432;Database=MetaDowIdentityServiceDb;...",
    "MetaDowProjectsDb": "Server=localhost;Port=5432;Database=MetaDowProjectsDb;...",
    "MetaDowSaaSDb": "Server=localhost;Port=5432;Database=MetaDowSaaSDb;..."
  }
}
```
...
The solution uses **centralized database migrations** via `Meta.Dow.DbMigrator`:
...
// Add reference to Meta.Dow.IdentityService.HttpApi.Client
...
4. Add service to `Meta.Dow.sln`
5. Register in `Meta.Dow.AppHost/Program.cs` with database and dependencies
6. Add service name constants to `Meta.Dow.Shared/MetaDowNames.cs`
7. Update gateway routing in `gateway/Meta.Dow.Gateway/appsettings.json`
8. Add DbContext to `Meta.Dow.DbMigrator`
...
OAuth2 clients are seeded in `Meta.Dow.DbMigrator` via `OpenIddictDataSeeder.cs`.

### Working with Multi-Tenancy

Multi-tenancy is enabled via `Meta.Dow.Shared/MultiTenancyConsts.cs`. Current value: `true`.
...
- `Meta.Dow.sln` - Main solution file
- `shared/Meta.Dow.Shared/MetaDowNames.cs` - Centralized service/database naming constants
- `apps/Meta.Dow.AppHost/Program.cs` - Aspire orchestration and startup sequencing
- `gateway/Meta.Dow.Gateway/appsettings.json` - YARP routing configuration
- `apps/Meta.Dow.AuthServer/OpenIddictDataSeeder.cs` - OAuth2 client seeding
- `shared/Meta.Dow.DbMigrator/` - Centralized database migration tool
...
- Run DbMigrator manually: `cd shared/Meta.Dow.DbMigrator && dotnet run`
...
- Check service names match `MetaDowNames` constants
